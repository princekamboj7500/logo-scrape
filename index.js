...
// const spreadsheetId = '1Lo5ZF0jBo8AnC9gK0WpoJJqrCkxFnTdjsThn4mYHv8A'; // Replace with the ID of your Google Sheet
// const range = 'Sheet1!A1'; // Replace with the desired sheet name and range
...
const data = [
    {
        "website": "http://atokaproperties.com",
    }
]

if(data.length){
    data.forEach(async function(d){
        const logoPath = await downloadImage(`https://logo.clearbit.com/:`+d.website);
        console.log(logoPath);
        // const resizelogoPath = await resizeImage(logoPath, './assets', new Date().getTime());
        // console.log(resizelogoPath);
        const overlayImagePath = await overlayImage(logoPath, './pattern/pattern.png', './assets');
        // await overlayImageTum('./BlankMockup/Kodiak_Mockup_Blank.png', overlayImagePath)
    });
}
...
const resizeImage = async (path_input, path_output, file_name) => {
...
const overlayImage = async(path_input, pattern_path, path_output) => {
    const saveTo  = path_output + "/engraved-" + new Date().getTime() + '.png';
...
const overlayImageTum = async(mockup, engraved_logo) => {
    console.log("Started OverlayImage")
